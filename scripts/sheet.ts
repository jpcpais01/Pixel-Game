...
// Usage: npx tsx scripts/sheet.ts [outDir] [scale] [wizard|void|pyro|warrior|jade|paladin|jedi|sith|fighter|monk|alchemist|witch|archer|storm]
...
const hero = arg === 'archer' || arg === 'storm' ? 'archer' : arg === 'alchemist' || arg === 'witch' ? 'alchemist' : arg === 'fighter' || arg === 'monk' ? 'fighter' : arg === 'warrior' || arg === 'jade' ? 'warrior' : arg === 'paladin' ? 'paladin' : arg === 'jedi' || arg === 'sith' ? 'jedi' : 'wizard';
...
const built: { anim: string; dir: string | null; canvas: { render(): ReturnType<ReturnType<typeof buildWizardFrames>[number]['canvas']['render']> } }[] =
  hero === 'archer' ? buildArcherFrames(arg === 'storm' ? STORM_LOOK : undefined) : hero === 'alchemist' ? buildAlchemistFrames(arg === 'witch' ? WITCH_LOOK : undefined) : hero === 'fighter' ? buildFighterFrames(arg === 'monk' ? MONK_LOOK : undefined) : hero === 'jedi' ? buildJediFrames(arg === 'sith' ? SITH_LOOK : undefined) : hero === 'warrior' ? buildWarriorFrames(arg === 'jade' ? JADE_LOOK : undefined) : hero === 'paladin' ? buildPaladinFrames() : buildWizardFrames(arg === 'void' ? VOID_LOOK : arg === 'pyro' ? PYRO_LOOK : undefined);
...
for (const layer of ['composite', 'diffuse', 'normal', 'emissive'] as const) {
  ...
  writeFileSync(`${out}/${arg === 'void' || arg === 'pyro' || arg === 'jade' || arg === 'sith' || arg === 'witch' || arg === 'storm' ? arg : hero}_${layer}.png`, encodePNG(W, H, img));
}
