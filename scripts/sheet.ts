...
// Usage: npx tsx scripts/sheet.ts [outDir] [scale] [wizard|void|warrior|paladin|jedi|sith]
...
const hero = arg === 'warrior' || arg === 'paladin' ? arg : arg === 'jedi' || arg === 'sith' ? 'jedi' : 'wizard';
...
const built: { anim: string; dir: string | null; canvas: { render(): ReturnType<ReturnType<typeof buildWizardFrames>[number]['canvas']['render']> } }[] =
  hero === 'jedi' ? buildJediFrames(arg === 'sith' ? SITH_LOOK : undefined) : hero === 'warrior' ? buildWarriorFrames() : hero === 'paladin' ? buildPaladinFrames() : buildWizardFrames(process.argv[4] === 'void' ? VOID_LOOK : undefined);
...
for (const layer of ['composite', 'diffuse', 'normal', 'emissive'] as const) {
  ...
  writeFileSync(`${out}/${arg === 'void' || arg === 'sith' ? arg : hero}_${layer}.png`, encodePNG(W, H, img));
}
