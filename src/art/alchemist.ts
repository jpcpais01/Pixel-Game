...
import { PixelCanvas, cyl, sphere, type RGB } from './pixel';
import { BEAK, BOOT, GLASS, GOLD, LEATHER, LENS, MANTLE, PLAGUE_COAT, PLAGUE_HAT, TOXIN, TOX_CORE, TOX_HOT, TOX_MID, TROUSER, WOOD } from './palette';
...
function heldFlask(c: PixelCanvas, x: number, y: number, size: 1 | 2, boil: number, bias: number, seed: number): void {
  ...
  c.part();
  c.ellipse(x, by + 0.45, r - 0.35, r - 0.55, TOXIN, { bias: bias + (boil > 0.6 ? 1 : 0) });
  c.part();
  ...
  c.spark(x, by, TOX_HOT, k * 0.6);
  c.spark(x - r, by, TOX_MID, k * 0.35);
  c.spark(x + r, by, TOX_MID, k * 0.35);
  c.spark(x, by + r, TOX_MID, k * 0.3);
  if (boil > 0) {
    for (let i = 0; i < 3; i++) {
      const up = ((seed * 3 + i * 5) % 7) + 1;
      c.spark(x + ((i + seed) % 3) - 1, neckTop - 1 - up * boil, i === 0 ? TOX_CORE : TOX_HOT, 0.55 * boil);
    }
  }
}
...
function arm(c: PixelCanvas, sx: number, sy: number, p: Placed, reach: number, hint: [number, number], hold: { size: 0 | 1 | 2; boil: number; seed: number }, bias = 0): void {
  ...
  c.part();
  c.capsule(sx, sy, ex, ey, 1.75, 1.5, PLAGUE_COAT, { bias });
  c.part();
  c.capsule(ex, ey, fx, fy, 1.5, 1.35, PLAGUE_COAT, { bias });
  if (hold.size) heldFlask(c, fx, fy - 0.6, hold.size, hold.boil, bias, hold.seed);
  c.part();
  c.ellipse(fx, fy, 1.35, 1.25, BOOT, { bias: bias + 1 });
}
...
function vial(c: PixelCanvas, x: number, y: number): void {
  ...
  c.px(x, y, TOXIN, sphere(0, 0.2));
  c.spark(x, y, TOX_MID, 0.3);
}

function lenses(c: PixelCanvas, pts: [number, number][], blink: boolean | undefined): void {
  c.part();
  for (const [x, y] of pts) {
    c.px(x, y, LENS, sphere(-0.3, 0.3), blink ? { bias: -2, glow: 0.35 } : {});
    if (!blink) c.spark(x, y, TOX_HOT, 0.35);
  }
}

/** The hat: a broad brim with the crown rising out of it, a plum band and a brass buckle. */
function hat(c: PixelCanvas, cx: number, U: number, view: View): void {
  const brimY = 9.4 + U;
  c.part();
  if (view === 'side') {
    c.ellipse(cx, brimY, 6.3, 1.45, PLAGUE_HAT, { normal: (_x, _y, dx, dy) => sphere(dx * 0.6, dy * 0.4 - 0.55, 1) });
  } else {
    c.ellipse(cx, brimY, 6.6, 1.95, PLAGUE_HAT, { normal: (_x, _y, dx, dy) => sphere(dx * 0.6, dy * 0.45 - 0.5, 1) });
  }
  ...
  }, PLAGUE_HAT, (_x, _y, t, u) => sphere(t * 0.9, u * 0.9 - 0.8, 1));
  c.part();
  c.shape(8 + U, 8 + U, () => [cx + off - 3.3, cx + off + 3.3], PLAGUE_COAT, (_x, _y, t) => cyl(t, 0.2), { bias: 1 });
...
function mantle(c: PixelCanvas, cx: number, U: number, l: number, r: number): void {
  ...
  }, MANTLE, (_x, _y, t, u) => sphere(t * 0.95, u - 0.6, 1));
}
...
function drawDown(c: PixelCanvas, p: Pose, seed: number): void {
  ...
  }, PLAGUE_COAT, (_x, y, t) => sphere(t * 0.9, y <= waist ? (y - top) / (waist - top) * 0.8 - 0.35 : 0.2, 1));
  ...
  c.part();
  for (const y of [19, 21]) c.px(cx - 2, y + U, GOLD, sphere(-0.3, -0.4));
  // Belt and buckle.
  c.part();
...
function drawUp(c: PixelCanvas, p: Pose, seed: number): void {
  ...
  c.part();
  c.ellipse(cx, 12 + U, 3.2, 2.6, PLAGUE_HAT, { normal: (_x, _y, dx, dy) => sphere(dx * 0.9, dy * 0.8 - 0.1, 1), bias: -1 });
  hat(c, cx, U, 'up');
...
function drawSide(c: PixelCanvas, p: Pose, seed: number): void {
  ...
  }, PLAGUE_COAT, (_x, y, t) => sphere(t * 0.9 - 0.1, y <= waist ? (y - top) / (waist - top) * 0.8 - 0.35 : 0.2, 1));
  ...
  c.part();
  c.ellipse(hx + 1.3, 12 + U, 1.9, 2.3, PLAGUE_HAT, { normal: (_x, _y, dx, dy) => sphere(dx * 0.9 + 0.2, dy * 0.8, 1) });
  c.part();
...
export function buildAlchemistFrames(): AlchemistFrame[] {
...
export function flaskFrame(i: number, big: boolean): PixelCanvas {
  ...
  c.ellipse(bx - ux * 0.3, by - uy * 0.3 + 0.2, r - 0.45, r - 0.5, TOXIN);
  c.part();
  ...
  c.spark(bx, by, TOX_HOT, big ? 0.7 : 0.5);
  if (big) {
    const cols: RGB[] = [TOX_HOT, TOX_MID];
    for (let k = 0; k < 6; k++) {
